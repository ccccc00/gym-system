...
public class AdminController {
    ...
    * */
    ...
    //会员管理页面
    @PostMapping("getVipInfoList")
    public Map<String,Object> getVipInfoList(@RequestBody Map<String,String> reqmap){
        Map<String,Object> map = new HashMap<>();
        //set original pagination
        int currentPage = 1;
        int currentCount = 5;
        String currentPageStr = reqmap.get("currentPage");
        String currentCountStr = reqmap.get("currentCount");
        ...
        String query_key = reqmap.get("query_key");
        if(query_key==null) query_key="";
        //invoke adminService for retrieve all vip info List
        PageBean<Gms_Vip> pageBean_venue = adminService.queryVipInfoList(currentPage, currentCount, query_key);
        //set and return response for thymeleaf html
        map.put("query_key", query_key);
        map.put("currentPage",pageBean_venue.getCurrentPage());
        map.put("currentCount",pageBean_venue.getCurrentCount());
        map.put("totalCount",pageBean_venue.getTotalCount());
     ...
     */
    @PostMapping("/addVip")
    public String addVip(@RequestBody Gms_Vip gms_vip){
         adminService.addVip(gms_vip);
         return "{}";
    }
    /**
     * 修改Vip金额
    ...
    public String updateVip(@RequestBody Gms_Vip gms_vip){
        adminService.updateVip(gms_vip);
        return "{}";
    }
    /**
     * 删除Vip
     ...
     */
    @PostMapping("/delVip")
    public String delVip(@RequestBody Gms_Vip gms_vip){
        adminService.delVip(gms_vip);
        return "{}";
    }
    /**
     * 完善Vip信息
     * @return
     */
    @PostMapping("/updatefVip")
    public String updatefVip(@RequestBody Gms_Vip gms_vip){
        adminService.updatefVip(gms_vip);
        return "{}";
    }
    @PostMapping("/searchVip")
    public Map<String,String> searchVip(@RequestBody Gms_Vip gms_vip){
        Map<String,String> map=new HashMap<>();
        Integer flag=adminService.searchVip(gms_vip);
        if(flag==0){
            map.put("state","1");
            map.put("info","更改成功");
        }else {
