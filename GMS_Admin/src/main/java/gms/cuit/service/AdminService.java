...
public class AdminService implements UserDetailsService {
    ...
    * */
    ...
    public PageBean<Gms_Vip> queryVipInfoList(int currentPage, int currentCount, String query_key) {
        PageBean pageBean = new PageBean();
        pageBean.setCurrentPage(currentPage);
        pageBean.setCurrentCount(currentCount);
        int totalCount = adminMapper.getVipTotalCountByKeyQuery(query_key);
        pageBean.setTotalCount(totalCount);
        int totalPage = (int) Math.ceil(1.0*totalCount/currentCount);
        pageBean.setTotalPage(totalPage);
        int index = (currentPage-1)*currentCount;
        //call Mapper service and get vip information list from database
        List<Gms_Vip> venue = adminMapper.queryVipList(index,currentCount,query_key);
        pageBean.setList(venue);
        return pageBean;
    }
    ...
    public void updateVip(Gms_Vip gms_vip) {
        adminMapper.updateVip(gms_vip);
    }
    public void addVip(Gms_Vip gms_vip) {
        int flag=adminMapper.searchVip(gms_vip);
        if(flag==0) adminMapper.addVip(gms_vip);
    }
    public void delVip(Gms_Vip gms_vip) {
        adminMapper.delVip(gms_vip);
    }
    public void updatefVip(Gms_Vip gms_vip) {
        //int flag=adminMapper.searchVip(gms_vip);
        //if(flag==1) adminMapper.updatefVip(gms_vip);
        adminMapper.updatefVip(gms_vip);
    }
    public int searchVip(Gms_Vip gms_vip) { return  adminMapper.searchVip(gms_vip);}
}
